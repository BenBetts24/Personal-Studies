...
def get_word_score(word, n):
    ...
    for char in word:
        letter_total += SCRABBLE_LETTER_VALUES[char]
        
    if length_score > 1:
        return (length_score*letter_total)
    else:
        return letter_total
...
def update_hand(hand, word):
    ...
    for char in word:
        if new_hand.get(char, 0) >= 1:
            new_hand[char] = new_hand.get(char, 1) - 1
    ...
    if word in word_list:
        ...
        for valid in word_list:
            for index in range(len(valid)):
                if (valid[index] != word[index]) and (word[index] != '*'):
                    testbool = False
                elif (word[index] == '*') and (valid[index] not in VOWELS):
                    testbool = False
                elif hand_copy.get(word[index], 0) <= 0:
                    testbool = False
                elif len(word) != len(valid):
                    testbool = False
                else:
                    testbool = True
                if testbool == False:
                    break
                hand_copy[word[index]] = hand_copy.get(word[index], 0) - 1
            if testbool == True:
                return True
            hand_copy = hand.copy()
        return False
    ...
    while calculate_handlen(hand) > 0:
        ...
        if user_input == "!!":
            ...
            if is_valid_word(user_input, hand, word_list):
                ...
                print('"', user_input, '" earned ', str(get_word_score(user_input, calculate_handlen(hand))), " points. Total: ", str(total_score), " points")
            else:
...
def substitute_hand(hand, letter):
    ...
    count = substitute_hand.get(letter, 0)
    replacement = ""
    
    while replacement == "" or (replacement in substitute_hand):
        replacement = random.choice(VOWELS + CONSONANTS)
    
    substitute_hand[replacement] = count
    substitute_hand[letter] = 0
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for x in range(int(hand_number)):
        hand = deal_hand(HAND_SIZE)
        replay_hand = True
        while replay_hand == True:
            display_hand(hand)
            if substitute_count > 0 and is_replayed == False:
    ...
    print("Total score of all hands: ", str(total_score), " points")
